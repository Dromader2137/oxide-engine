...
use ecs::World;
use rendering::{EventLoop, Mesh, Renderer, ShaderData, ShaderManager, ShaderType, VPData, Window};
...
pub fn run(mut world: World) {
    ...
    let mut vp_buffer: UpdatableBuffer<VPData> =
        UpdatableBuffer::new(&renderer, BufferUsage::UNIFORM_BUFFER);
        ...
        .iter_mut() 
    {
        ...
        transform
            ...
            .as_mut()
            .unwrap()
            .write(Matrix4f::translation(position.to_vec3f()) * Matrix4f::rotation_yxz(rotation) * Matrix4f::scale(scale));
    }
    ...
    event_loop
        ...
        .run(move |event, _, control_flow| match event {
            ...
            Event::MainEventsCleared => {
                renderer.handle_possible_resize(
                    &window,
                    &mut vp_data,
                    &vp_buffer,
                    &mut world,
                ...
                now = Instant::now();
                vp_data.view = Matrix4f::look_at(
                    Vec3f::new([0.0, 0.0, 0.0]),
                    Vec3f::new([dbg.sin(), 0.0, dbg.cos()]),
                    Vec3f::new([0.0, 1.0, 0.0]),
                );
                vp_buffer.write(vp_data);
            }
